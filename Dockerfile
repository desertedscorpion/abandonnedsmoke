...
RUN dnf install --assumeyes jenkins git vagrant openssh-server tar npm nodejs docker && dnf clean all && systemctl enable jenkins.service && systemctl enable sshd.service
RUN adduser slave
